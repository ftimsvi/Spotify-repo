...
@artist.route('/artist/<int:artist_id>/followers')
def getArtistFollowers(artist_id):
    conn = get_db_connection()
    cur = conn.cursor()
...
@artist.route('/artist/<int:artist_id>/followings')
def getArtistFollowings(artist_id):
    conn = get_db_connection()
    cur = conn.cursor()
...
@artist.route('/artist/<int:artist_id>/follow/<int:followed_id>', methods=['GET', 'POST'])
def followArtist(artist_id, followed_id):
    if request.method == 'POST':
        token = request.headers.get('Authorization').split()[1]
        if not token:
            return jsonify({'error': 'Missing token'}), 400
...
@artist.route('/artist/<int:artist_id>/followUser/<int:followed_id>', methods=['GET', 'POST'])
def followUser(artist_id, followed_id):
    if request.method == 'POST':
        token = request.headers.get('Authorization').split()[1]
        if not token:
            return jsonify({'error': 'Missing token'}), 400
        ...
        try:
            ...
            cur.execute(
                'SELECT * FROM USERS WHERE user_id = %s AND is_artist = %s', (artist_id, False))
            user = cur.fetchone()
